...
for repeat_i in range(2):
    rawfile = f"H:\\ctzdata\\W21-1\\20240828_Ene_Calib_500fps_30mins_-700V_Am241_Co57\\20240828_Ene_Calib_500fps_30mins_-700V_Am241_Co57repeat_{repeat_i:03d}\\20240828_Ene_Calib_500fps_30mins_-700V_Am241_Co57.bin"
    rawfiles.append(rawfile)
output_filepath = r"H:\\ctzdata\\W21-1\\20240828_Ene_Calib_500fps_30mins_-700V_Am241_Co57_results"

# for repeat_i in range(2):
#     rawfile = f"H:\\ctzdata\\W21-1\\20240829_Ene_Calib_500fps_30mins_-700V_Am241_Co57\\20240829_Ene_Calib_500fps_30mins_-700V_Am241_Co57repeat_{repeat_i:03d}\\20240829_Ene_Calib_500fps_30mins_-700V_Am241_Co57.bin"
#     rawfiles.append(rawfile)
crystals_frames_folder = r"H:\ctzdata\W21-1\processed\crystals_frames"
crystals_spectra_folder = r"H:\ctzdata\W21-1\processed\crystals_spectra"
crystals_calibrations = [r"H:\ctzdata\W21-1\processed\calibration_crystal_0.h5"]
...
raw2clusters(rawfiles, output_filepath, crystals_calibrations, crystals_thresholds, extended_mode=False, config="config_czt.txt")

#%% peak finding test

# spectra_file = r"H:\ctzdata\W21-1\processed\crystals_spectra\pixels_spectra_crystal_0.h5"
# with h5py.File(spectra_file, 'r') as f:
#     bins = f['bins'][:]
#     pixels_spectra = f['pixels_spectra'][:]

# for pixel_id in range(964, pixels_spectra.shape[0]):
#     peaks_indices = find_peaks(bins, pixels_spectra[pixel_id:pixel_id+1, :], n_peaks=2, savitzky_kernel_half_length=4, smooth_kernel_half_length=2, neglected_bins=100, high_end_neglected_bins=200, peak_distance=20, verbose=True)
...
# peaks_indices = find_peaks(bins, pixels_spectra, n_peaks=2, savitzky_kernel_half_length=4, smooth_kernel_half_length=2, neglected_bins=150, high_end_neglected_bins=200, peak_distance=20, verbose=True)
# pixels_peaks0 = peaks_indices[:, 0]
# pixels_peaks1 = peaks_indices[:, 1]
# pixels_peaks0_adu = bins[pixels_peaks0]
# pixels_peaks1_adu = bins[pixels_peaks1]
...
#     print(f"Fitted pixel {pixel_id+1}/{pixels_spectra.shape[0]}")
...
# with h5py.File(r"H:\ctzdata\W21-1\processed\pixels_peak0_crystal_0.h5", 'w') as f:
#     f['peak0'] = pixels_peak0_params
# with h5py.File(r"H:\ctzdata\W21-1\processed\pixels_peak1_crystal_0.h5", 'w') as f:
...
# with h5py.File(r"H:\ctzdata\W21-1\processed\pixels_peak0_crystal_0.h5", 'r') as f:
#     pixels_peak0_params = f['peak0'][:]
# with h5py.File(r"H:\ctzdata\W21-1\processed\pixels_peak1_crystal_0.h5", 'r') as f:
#     pixels_peak1_params = f['peak1'][:]
# pixels_peaks0_adu = pixels_peak0_params[:, 0]
# pixels_peaks1_adu = pixels_peak1_params[:, 0]

# pixels_start_adus = np.argmax(pixels_spectra[:, 0:], axis=1)
# energy1 = 59.54  # keV for Am241
# energy2 = 122.06  # keV for Co57
# slopes, intercepts, isvalids, thresholds, R2 = calibrate_peaks(pixels_peaks0_adu, pixels_peaks1_adu, energy1, energy2, \
#         threshold_energy=5, peaks1_start_adus=pixels_start_adus, peaks2_start_adus=pixels_start_adus, verbose=True)

# fig = plt.figure(figsize=(8, 6))
# ax = plt.imshow(pixels_peaks0_adu[:,0].reshape(40,40), cmap='gray')
...
# calibration_filepath = r"H:\ctzdata\W21-1\processed\calibration_crystal_0.h5"
# with h5py.File(calibration_filepath, 'w') as f:
#     f.create_dataset('pixels_calibrations', data=np.vstack((slopes, intercepts)).T)
#     f.create_dataset('pixels_isvalids', data=isvalids)
#     f.create_dataset('pixels_thresholds', data=thresholds)
#     f.create_dataset('pixels_R2', data=R2)

#%% after the generation of calibration file, visualize the results
...
# calibration_file = r"H:\ctzdata\W21-1\processed\calibration_crystal_0.h5"
...
# interactive.add_subplot(0, {"Peak1": pixels_peaks0_adu, "Peak2": pixels_peaks1_adu, "Gain": slopes.reshape(40,40), "Offset": intercepts.reshape(40,40), 'Isvalid': isvalids.reshape(40,40)}, multiple_spectra={"Am241": pixels_spectra, "Co57": pixels_spectra}, \
#                          multiple_peaks={"Am241": pixels_peaks0_adu, "Co57": pixels_peaks1_adu}, multiple_energies={"combined": [energy1, energy2]})
...
# with h5py.File(r"H:\ctzdata\W21-1\processed\pixels_thresholds_crystal_0.h5", 'w') as f:
